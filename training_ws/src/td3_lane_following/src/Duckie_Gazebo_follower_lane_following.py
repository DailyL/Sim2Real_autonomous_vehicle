...
class Env(object):
    ...
    def setReward(self,action):
        ...
        if abs(action[1]*2.0) >= 2.0:
            r_cmd_phi = - (abs(action[1]*2.0)-2)
        else:
            r_cmd_phi = 0 
        ...
        """
        if off_road or in_small or r_on_left:
            reward = -1 
        else:
            reward = (self.follower_v*(np.cos(self.lanePose_phi)-self.lanePose_d) + r_v + r_cmd_phi)*0.4
        """
        if off_road:
            reward = -3 
        else:
            reward = (0.25*(1-r_d/0.2)+0.4*r_v)
    ...
    #return: state, reward, done
        ...
        car_twist_msg = Twist()
        car_twist_msg.linear.x = (action[0]+1.0)/2.0  # into [0,1]
        car_twist_msg.angular.z = action[1]*2.0     # into [-4,4]  
        ...
        status8 = 0
        status9 = 0
        status10 = (action[0]+1.0)/2.0
        status11 = action[1]*2.0
