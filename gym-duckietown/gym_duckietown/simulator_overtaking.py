...
REWARD_INVALID_POSE = -1

MAX_SPAWN_ATTEMPTS = 50000

LanePosition0 = namedtuple('LanePosition', 'dist dot_dir angle_deg angle_rad closest_point desired_heading')
...
class Simulator_overtaking(gym.Env):
    """
    ...
    def reset(self):
        ...
        if "start_pose" in self.map_data:
            self.start_pose = self.map_data["start_pose"]
    ...
    def get_lane_pos2(self, pos, angle):
        ...
        return LanePosition(dist=signedDist, dot_dir=dotDir, angle_deg=angle_deg,
                            angle_rad=angle_rad, closest_point=point, desired_heading=tangent)
    ...
    def _valid_pose(self, pos, angle, safety_factor=0.1):
        """
    ...
    def get_agent_info(self):
        ...
        if self.full_transparency:
            ...
            for obj in self.objects:
                if not obj.static and obj.kind == "duckiebot":
                    vehicle_position = obj.pos
            info['Other_vehicle'] = vehicle_position
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        agent_corners = get_agent_corners(pos, angle)
        _collision = self._collision(agent_corners)

        in_left = False
        v_desired = 0.5
        distance_to_other = 10 
        ...
        distance_to_other = distance.euclidean(pos, vehicle_position)
        ...
        try:
            ...
            # Compute the reward
            """
            components: lane folloing reward + velocity + heading
            """

            e_y = lp.dist
            x_d = 1 * math.atan(0.5*e_y)
            
            r_yaw = lp.angle_rad

            if _collision or (done_collision and lp.dist < 0.23):
                reward = -1
            else:
                r_v = (speed - v_desired) / (speed + 0.00000001)
    ...
    def step(self, action: np.ndarray):
        ...
        obs = self.render_obs() # get image 

        # get feature based obs
    ...
    def _compute_done_reward(self) -> DoneRewardInfo:
        ...
        distance_to_other = distance.euclidean(self.cur_pos, vehicle_position)
        
        done_collision = 0
      
        
        if not self._valid_pose(self.cur_pos, self.cur_angle) or done_collision:
